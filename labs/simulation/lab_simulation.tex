...
\begin{document}
	...
	\section{Background}
	  ...
	  \subsection{Simulation: The Rumor}
	...
	\section{Activity \#2}
		...
		\subsection{Exercises}
			\begin{enumerate}
				...
				\item Create the method, \code{estimateExpectedValue()}, that will accept \code{n}, the number of guests (including Bob) invited to Alice's party, and run the simulation in order to return the average number of people to hear the rumour.\\
		...
		\subsection{Questions}
			\QBox{Briefly explain how you chose the number of times to run the simulation for each \code{estimateProbability()} and \code{estimateExpectedValue().}}{4cm}
			\ \\[9pt]
			\QBox{How does varying the value of \code{n} affect the values returned by \code{estimateProbability()} and \code{estimateExpectedValue()}. Discuss trends rather than specific values.}{4cm}
...
\pagebreak
	...
	\section{Final Analysis}
		...
		\ \\[9pt]
		\QBox{Which part of implementing \code{runSimulation()}, \code{estimateProbability()}, or \code{estimateExpectedValue()} did you find most challenging? How did you overcome these challenges?}{4cm}
		\ \\[9pt]
