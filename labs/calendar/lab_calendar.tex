...
\begin{document}
	...
	\section{Background}
		...
		\subsection{Examples}
			\EBox{July 9, 1983}{
				\[
					w = \left(9 + \floor{2.6(5) - 0.2} + 83 + \floor{\dfrac{83}{41}} + \floor{\dfrac{19}{4}} - 2(19)\right) \mathbf{mod}\ 7 = 6
				\]
	...
	\section{Applications}
		...
		\ \\[9pt]
		\QBox{Explain what happens when you attempt to calculate the day of the week for a non-existent day (February 29, 2017 or July 33, 2020). Is the result consisitent with what you would expect for this kind of formula? Explain why or why not.}{4cm}
		\ \\[9pt]
	...
	\section{Activity \#1}
		...
		\subsection{Exercises}
			\begin{enumerate}
				...
				\item Implement the \code{getDayOfWeek()} method. This method should take as parameters the \code{day}, \code{month}, and \code{year} for the desired date and return the day of the week as a string (``Monday'', ``Tuesday'', etc.).
		...
		\subsection{Questions}
			...
			\ \\[9pt]
			\QBox{Why do you think it is considered ``better practice'' to have used the \code{calculateDayOfWeek()} method within the implementation of \code{getDayOfWeek()} rather than just repeating the use of the formula?}{5cm}
	...
	\section{Activity \#2}
		...
		\subsection{Exercise}
			\begin{enumerate}
				\item Implement the \code{isLeapYear()} method which will take a year as input and produce \code{true} if the given year is a leap year and \code{false} otherwise.
			...
			{\small\textbf{Note:} Unlike the formula you used for \code{calculateDayOfWeek()}, your implementation of \code{printCalendar()} will have to use \code{isLeapYear()} in order to manually handle February's calendar output.}
			\end{enumerate}
