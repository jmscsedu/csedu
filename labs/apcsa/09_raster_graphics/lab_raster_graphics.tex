...
\begin{document}
  ...
  \section{Activity \#1} %Reading/Writing Image Files
    ...
    \subsection{Exercises}
      \begin{enumerate}
        \item Implement the \code{readImageFile()} method with the following requirements:
          \begin{enumerate}
            ...
            \item If the image type is \code{CSBM}, a colour palette should be created containing only two entries, one for \code{Color.BLACK} and one for \code{Color.WHITE}.
          ...
          \end{enumerate}
        \item Implement the \code{writeImageFile()} method with the following requirements:
          \begin{enumerate}
