...
\begin{document}
  ...
  \pagebreak
  \section{Activity \#1} %Reading/Writing Image Files
    ...
    \subsection{Exercises}
      \begin{enumerate}
        \item Implement the \code{openImageFile()} method with the following requirements:
          \begin{enumerate}
            \item Instantiates and returns an \code{Image} class object, populating all of its relevant instance variables, as indicated below.
            \item The \code{width} and \code{height} of the \code{Image} should be read from the image file.              
            \item If the image type is \code{CSBM}, the colour palette object should remain \code{null}; the \code{hasPalette} instance variable should be instantiated appropriately.
            \item If the image is type \code{CSIC}, a colour palette should be read from the file containing the appropriate number of colours; the \code{hasPalette} instance variable should be instantiated appropriately.
            \item The \code{pixels} attribute of the \code{Image} should be created as a two-dimensional array of appropriate values.\\
            {\small\textbf{Note:} \code{pixels.length} should be equal to the \emph{width} of image and \code{pixels[i].length} should be equal to the \emph{height} for each $i < width$.}
          \end{enumerate}
        \item Implement the \code{saveImageFile()} method with the following requirements:
          \begin{enumerate}
            \item Reads relevant information from the given \code{Image} object.
            \item Selects which file format to use (\code{CSBM} or \code{CSIC}).
  ...
  \section{Activity \#2} %Defining new colour palettes...
    \subsection{Introduction}
      %The ColorPreset class.
    \subsection{Exercises}
      \begin{enumerate}
        \item Implement the \code{listColourPresets()} method. This method should populate the \code{colourPresets} \code{List} with each of the colour palettes in the ``ColourPresets'' file.
        \item Implement the \code{selectColourPreset()} method. This method should update the currently displayed \code{Image} to use the selected preset colour palette.
        \item Implement the \code{saveColourPreset()} method. This method should append the current palette to the ``ColourPresets'' file.
      \end{enumerate}
    \subsection{Questions}
