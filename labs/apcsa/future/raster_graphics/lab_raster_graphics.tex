    ...
    \subsection{Exercises}
      \begin{enumerate}
        \item Implement the \code{openImageFile()} method with the following requirements:
          \begin{enumerate}
            \item Instantiates and returns an \code{RGImage} class object, populating all of its relevant instance variables, as indicated below.
            \item The \code{width} and \code{height} of the \code{RGImage} should be read from the image file.              
            \item If the image type is \code{CSBM}, a colour palette should be created containing only two entries, one for Color.BLACK and one for Color.WHITE.
            \item If the image is type \code{CSIC}, a colour palette should be read from the file containing the appropriate number of colours.
            \item The \code{pixels} attribute of the \code{RGImage} should be created as a two-dimensional array of appropriate values.\\
        ...
        \item Implement the \code{saveImageFile()} method with the following requirements:
          \begin{enumerate}
            \item Reads relevant information from the given \code{RGImage} object.
            \item Selects which file format to use (\code{CSBM} or \code{CSIC}) based on the palette present within the \code{RGImage}.
